...
# 編譯 (只能在Node14 這邊用Docker示範)

移動到專案根目錄

```bash
docker run -it --rm  -v ./:/app --workdir="/app" node:14 bash
```
PS.不要使用docker --user 選項，會有沒辦法npm link的權限問題

進入container之後

1. 切換至simple-mind-map

```bash
cd /app/simple-mind-map
npm install --save
npm link
```
...
npm run build
```
編譯完成將會產生在專案根目錄dist資料夾及index.html檔案。
